...
export default function UserGalleryView({ items = [] }) {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {items.length === 0 ? (
        <div className="text-center py-12 text-sky-400">No images available in the gallery.</div>
      ) : (
          ...
          {items.map((item) => (
            <div key={item.id} className="group relative overflow-hidden rounded-lg border border-sky-100 bg-white shadow-sm transition hover:shadow-md flex flex-col justify-between">
              <div>
                ...
                {/* Qaybta qoraalka ee la saxay */}
                <div className="p-4 space-y-2">
                  {/* line-clamp-1 waa laga saaray halkan si title-ka oo dhan u muuqdo */}
                  <h4 className="font-bold text-sky-950 text-base break-words">
                    {item.title}
                  </h4>
                  {/* line-clamp-1 halkan xata waa laga saaray si magaca file-ka uusan u go'in */}
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 animate-fade-in">
          <div className="relative max-w-3xl w-full bg-white rounded-lg overflow-hidden shadow-2xl">
            ...
            <img src={selectedImage.image} alt={selectedImage.title} className="w-full max-h-[70vh] object-contain bg-sky-950" />
